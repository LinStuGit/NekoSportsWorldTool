...
`./android/build.ps1 -Lite` 构建精简包（`NekoSportsWorldTool-*-lite.apk`，Rust 侧 `--features android,lite`）：

- 仅支持广西职业技术大学；跑步轨迹沿内置校园离线路网规划，全程无任何路网 API 调用。
- Material Design 风格界面：顶部应用栏 + 底部导航（首页 / 设置 / 日志 / 关于）四个子页面。
  - 首页：未登录时是登录页；登录后展示本次跑步方案（距离/配速/用时/开始）与「开始跑步」主按钮，「换一版」重掷方案。
  - 设置：运动参数（距离/配速/海拔/人脸校验）、开始时间（随机/指定、日期）、账号与设备（含登出）。
  - 日志 / 关于：运行日志清空与查看；版本与自更新、声明。
- 启动后自动读取本机设备信息与 GPS 定位（城市/锚点）写入设备身份；标准包的权限与页面不受影响。
